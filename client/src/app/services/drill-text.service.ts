...
export class DrillTextService {
  ...
  getRandomDrillWords(difficulty: DrillDifficulty, drillLength: DrillLength): string[] {
    ...
    const shuffledCommonWords = [...basicWords].sort(() => Math.random() - 0.5);
    const shuffledDifficultyWords = [...difficultyWords].sort(() => Math.random() - 0.5);
    
    const selectedCommonWords = shuffledCommonWords.slice(0, commonWordsCount);
    const selectedDifficultyWords = shuffledDifficultyWords.slice(0, difficultyWordsCount);

    // Combine and shuffle the final result
    const combinedWords = [...selectedCommonWords, ...selectedDifficultyWords];
    return combinedWords.sort(() => Math.random() - 0.5);
  }
